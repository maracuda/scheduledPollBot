...
public class S3ScheduledPollRepository : IScheduledPollRepository
{
    ...
    {
        var allPolls = await s3ClientWrapper.ReadAllAsync<ScheduledPollDbo>(bucketName, key);
        await s3ClientWrapper.WriteAsync(allPolls.Append(poll).ToArray(), bucketName, key);
    }
    ...
    {
        var allPolls =
            (await s3ClientWrapper.ReadAllAsync<ScheduledPollDbo>(bucketName, key)).ToDictionary(p => p.Id);
